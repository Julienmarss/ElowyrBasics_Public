...
public class JoyauxGiveCommand {

    private final JoyauxManager joyauxManager = JoyauxManager.getInstance();

    @Command(name = "joyaux.give", permission = "elowyrbasics.joyaux.give")
    public void onCommand(CommandArgs args) {
        CommandSender sender = args.getSender();

        if (args.length() != 2) {
            sender.sendMessage(Utils.color(joyauxManager.getConfig().getString("MESSAGES.GIVE.USAGE")));
            return;
        }
        ...
        sender.sendMessage(Utils.color(joyauxManager.getConfig().getString("MESSAGES.GIVE.SUCCESS").replace("%player%", target.getName()).replace("%quantity%", String.valueOf(quantity))));
        if (target.isOnline()) {
            target.getPlayer().sendMessage(Utils.color(joyauxManager.getConfig().getString("MESSAGES.GIVE.SUCCESS-OTHER").replace("%quantity%", String.valueOf(quantity))));
        }
    }
}
